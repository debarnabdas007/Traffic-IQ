...
# change video file for training data collection
VIDEO_FILENAME = 'video6.mp4' 

# Constants
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) # My Project root
...
def collect_data():
    ...
    while True:
        ...
        # defining the kernel: a 5x5 pixel square
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5)) 

        # For closing holes (*Fill gaps inside objects)
        mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel)
        
        # Dilation (*Make objects fatter to merge close parts)
        mask = cv2.dilate(mask, kernel, iterations=2) 
   

        
        # Stabilization Logic (Running MOG2 blindly after a jump)
        ...
        for cnt in contours:
            ...
            if area > MIN_AREA: # filtering small objects, only large ones considered
        ...
        # Waits only if we found a valid object !!
        ...
        if key == ord('q'):
            ...
            # Now Resetting MOG2 for new lighting
            object_detector = cv2.createBackgroundSubtractorMOG2(history=100, varThreshold=40)
