...
''' We must scale new frames exactly like training. If scaler missing -> predictions wrong.
so putting error handling is crucial here'''
...
def generate_frames():
    ...
    while True:
        ...
        try:
            ...
            roi = frame[250:600, 0:1020] # we process only bottom half where vehicles are. Top part has sky.
            mask = object_detector.apply(roi) # Background substraction
            ...
            # Each contour = possible vehicle
            contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            cv2.line(frame, (0, LINE_Y), (1020, LINE_Y), (255, 255, 255), 2)

            for cnt in contours:
                area = cv2.contourArea(cnt)
                if 800 < area < 80000: # too small : Noise vs too large : Camera glitch
                    x, y, w, h = cv2.boundingRect(cnt)
                    y += 250
                    cx, cy = x + w//2, y + h//2
                    aspect_ratio = float(w) / h
                    
                    feat = scaler.transform([[area, aspect_ratio]])  # ** need scaler before applying rediction
                    pred_idx = model.predict(feat)[0]
                    label = {0: "Car", 1: "Truck", 2: "Bike"}[pred_idx]

                    # some RULE-BASED corrections
                    if label == "Truck" and area < 5000: label = "Car"
                    if label == "Bike" and area > 3000: label = "Car"

                    # Count logic for the Line
            ...
            #Convert Frame to JPEG ( Browser only understand JPRG )
            ret, buffer = cv2.imencode('.jpg', frame)
            yield (b'--frame\r\n' b'Content-Type: image/jpeg\r\n\r\n' + buffer.tobytes() + b'\r\n') # Streaming to Browser
            
        except Exception as frame_err:
            logger.warning(f" Error processing frame: {frame_err}")
            continue
